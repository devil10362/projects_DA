...
plt.tight_layout()
plt.show()
model2 = ["VGG-Face", "Facenet", "Facenet512", "OpenFace", "DeepFace", "DeepId", "ArcFace", "Dlib", "SFace"]
...
for mode in models:
    try:
        ...
        fig.suptitle(f"{model2} → Verified: {result['verified']} | Distance: {result['distance']:.4f}", fontsize=14)
        plt.show()

    except Exception as e:
        print(f"Model2: {model2} | Error: {e}")
...
print("[INFO] Model saved as emotion.h5")
